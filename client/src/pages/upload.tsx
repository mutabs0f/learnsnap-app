...
export default function UploadPage() {
  ...
  const processImages = async () => {
    ...
    try {
      ...
      const newPagesRemaining = pagesRemaining - capturedImages.length;
      setPagesRemaining(newPagesRemaining);
      localStorage.setItem("pagesRemaining", String(newPagesRemaining));
      ...
      if (error instanceof ApiError && error.status === 402) {
        ...
        const errorMessage = error instanceof Error ? error.message : "فشل في معالجة الصور";
        toast({
          title: "حدث خطأ",
