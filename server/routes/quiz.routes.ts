      ...
      if (authHeader && authHeader.startsWith("Bearer ")) {
        ...
        try {
          ...
          if (!session) {
            ...
            logger.warn("Quiz create with invalid session token", {
              deviceId: data.deviceId.substring(0, 8),
            });
            return res.status(401).json({
          ...
          if (new Date(session.expiresAt) < new Date()) {
            ...
            logger.warn("Quiz create with expired session token", {
              deviceId: data.deviceId.substring(0, 8),
          ...
          await clearIdempotency(idemKey);
          logger.error("Session validation error in quiz create", { error: (e as Error).message });
          return res.status(401).json({
