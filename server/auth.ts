...
// [SECURITY FIX v4.2] Require JWT_SECRET in production - no fallback allowed
const JWT_SECRET = (() => {
  const secret = process.env.JWT_SECRET || process.env.SESSION_SECRET;
  if (!secret && process.env.NODE_ENV === 'production') {
    console.error('FATAL: JWT_SECRET or SESSION_SECRET is required in production');
    process.exit(1);
  }
  return secret || 'learnsnap-development-secret-key-min-32-chars';
})();
...
export function requireChildAccess(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  if (parentToken) {
    try {
      ...
      // Allow parent-level routes but block child-specific routes
