...
// [SECURITY FIX v3.3.3] Require JWT_SECRET in ALL environments - no fallback
const JWT_SECRET = (() => {
  const secret = process.env.JWT_SECRET || process.env.SESSION_SECRET;
  if (!secret) {
    // Allow explicit dev bypass only
    ...
    console.error('Set JWT_SECRET or SESSION_SECRET environment variable');
    console.error('For development only, set ALLOW_DEV_JWT_FALLBACK=true');
    process.exit(1);
  }
  return secret;
})();
...
export async function requireChildAccess(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  if (parentToken) {
    try {
      ...
      if (childId) {
        // [SECURITY FIX v3.3.3] Use await to prevent race condition
        try {
      ...
      // [SECURITY FIX v4.2] Parent authenticated but no childId - check if route requires it
      const childRequiredPaths = ['/quiz', '/lesson', '/progress', '/results', '/child-'];
      const requiresChild = childRequiredPaths.some(p => req.path.includes(p));
