      ...
      try {
        ...
        if (!user) {
          ...
          if (user) {
            ...
            // Give early adopter bonus pages
            if (isEarlyAdopter) {
              try {
                // Get or create device ID from session/cookie
                const tempDeviceId = `google_${user.id}`;
                await storage.addPageCredits(tempDeviceId, EARLY_ADOPTER_FREE_PAGES);
                await storage.linkDeviceToUser(tempDeviceId, user.id);
                logger.info("✅ Early adopter bonus granted", { 
                  userId: user.id, 
                  pages: EARLY_ADOPTER_FREE_PAGES,
                  userNumber: userCount + 1
                });
              } catch (bonusError) {
                logger.warn("Could not grant early adopter bonus", { error: (bonusError as Error).message });
              }
            }
    ...
    try {
      ...
      if (deviceId) {
        try {
          ...
          // Give early adopter bonus pages
          if (isEarlyAdopter) {
            await storage.addPageCredits(deviceId, EARLY_ADOPTER_FREE_PAGES);
            logger.info("✅ Early adopter bonus granted", { 
              userId: user.id, 
        ...
        // Even without deviceId, create credit record for early adopter
        try {
          const tempDeviceId = `email_${user.id}`;
          await storage.addPageCredits(tempDeviceId, EARLY_ADOPTER_FREE_PAGES);
          await storage.linkDeviceToUser(tempDeviceId, user.id);
          logger.info("✅ Early adopter bonus granted (no device)", { 
            userId: user.id, 
            pages: EARLY_ADOPTER_FREE_PAGES,
            userNumber: userCount + 1
          });
